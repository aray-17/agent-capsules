...
without per-model configuration. Paper §6.
...
minimum detectable difference. Paper §9.3.
...
once the rolling-mean window has saturated. Paper §11.5.
...
drop 27%–42%. Paper §11.5(b).

## Negative result

**N1. Forced context injection does not rescue compound quality.**
Injecting more context into a merged call worsens compression
rather than relieving it. The paper's escalation ladder therefore
recovers quality by moving *toward* per-agent dispatch, not by
rewriting merged prompts. Paper §8.
